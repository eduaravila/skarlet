<script>
import { metatags, page, url } from '@roxi/routify';
...
const siteTitle = `${title} ${titleSeparator}`;
const canonicalURL = new URL($url(), process.env.SITE);
const socialImageURL = new URL(ogImage ? ogImage : "/social-card.png", $url()).href;
